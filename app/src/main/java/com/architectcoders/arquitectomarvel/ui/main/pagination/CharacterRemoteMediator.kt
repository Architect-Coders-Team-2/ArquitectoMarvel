...
class CharacterRemoteMediator @Inject constructor(
    ...
    override suspend fun initialize(): InitializeAction {
        ...
        val lastTimeStamp = getLastTimeStampFromCharacterEntity(Unit) ?: -1L
    ...
    override suspend fun load(
        ...
        lastMediatorResult = try {
            ...
            val response = getRemoteCharacters(nextPageNumber.times(REQUEST_LIMIT))
            val characterList = response.characterData?.characters
            if (characterList.isNullOrEmpty()) {
                if (response.characterData?.total == getLocalCharactersCount(Unit)) {
                ...
                if (loadType == LoadType.REFRESH && nextPageNumber == 0) {
                    deleteAllLocalCharacters(Unit)
                }
                characterList.map { it.pageNumber = nextPageNumber }
                insertAllLocalCharacters(characterList)
